...
if ($actions == 'deleteThisBook' && $code == 'e102') {
    $sql = 'DELETE FROM [cpw_library].[dbo].[book_data] WHERE book_id = '.$book_id.' OR book_accession_no = '.$book_accession_no.'';
    $stmt = sqlsrv_query($conn, $sql);
    if ($stmt === false) {
        echo 'dCOM101ps';
        die(print_r(sqlsrv_errors(), true));
    }
    $sql = 'DELETE FROM [cpw_library].[dbo].[book_Author_Translator] WHERE book_id = '.$book_id.' ';
    $stmt = sqlsrv_query($conn, $sql);
    if ($stmt === false) {
        echo 'dCOM101ss';
        die(print_r(sqlsrv_errors(), true));
    ...
    echo 'COM101as';
}  elseif ($actions == 'editdetailThisBook' && $code == 'e101') {
...
//    if($book_id == ''){ // insert
    if ($book_picture != '') {
        ...
        $sql = "UPDATE book_data SET  book_picture = '$FileNew' WHERE book_id = '$book_id'";
        $stmt = sqlsrv_query($conn, $sql);
        if ($stmt === false) {
            die(print_r(sqlsrv_errors(), true));
        }
...
book_accession_no = '$book_accession_no'
, book_call_classification_100 = '$book_call_classification_100'
...
, book_public_series = '$book_public_series'
, book_public_copy = '$book_public_copy'
, book_isbn = '$book_isbn'
, book_name = '$book_name'
...
, book_number_print = '$book_number_print'
...
WHERE book_id = '$book_id'";
    ...
    $sql = "DELETE  FROM book_languge where  book_id = '$book_id'";
    $stmt = sqlsrv_query($conn, $sql);
    if ($stmt === false) {
        die(print_r(sqlsrv_errors(), true));
    }
    ...
    $sql = "DELETE  FROM book_Author_Translator where  book_id = '$book_id'";
    $stmt = sqlsrv_query($conn, $sql);
    if ($stmt === false) {
        die(print_r(sqlsrv_errors(), true));
    }
    ...
    echo '<script>window.location="../../detail_book.php?book_id=' . $book_id . '"</script>';
}
